...
<?php //dd($polling_stations);?>
	...
	<?php 
		...
		foreach ($polling_stations as $value) {
			 ...
			 $value->poll_building = str_replace("'", "&#39;", $value->poll_building);
			 $value->poll_building = str_replace('"', "&#34;", $value->poll_building);
			if($value->poll_type == "Auxiliary"){
				...
				$model[] .= "['".trim(str_replace("\n", "", $value->poll_building))."', ".trim($value->latitude).", ".trim($value->longitude).", ".$i."]";
			}else{
				
					$notified[] .= "['".trim(str_replace("\n", "", $value->poll_building))."', ".trim($value->latitude).", ".trim($value->longitude).", ".$i."]";
