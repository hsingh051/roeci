...
@section('content')
	...
	<?php $selectedPoll= (isset($_GET['pollStation']))? $_GET['pollStation'] : "";
	if($selectedPoll==""){ ?>
		<style>
			.hideClass{
				display: none;
	...
	<div class="container-widget">
		<div class="row">
			<div class="col-md-12">
				<div class="searchBar">
					<form class="topSearchForm electTopSrch" enctype="multipart/form-data" method="get" action="{{url('ro/electoral-rolls-submit') }}">
					<input type="hidden" name="_token" value="<?= csrf_token(); ?>">
						<div class="panel-title pageTitle">Electoral Rolls</div>
						<div class="form-group{{ $errors->has('pollStation') ? ' has-error' : '' }}">
							<select name="pollStation" class="form-control">
								<option value="">Select Polling Station</option>
								@foreach($poll_station as $poll_stations)
								<option value="{{ $poll_stations->ps_id }}" <?php if(($poll_stations->ps_id)==($selectedPoll)){ echo "selected"; } ?> >{{ $poll_stations->poll_building }} {{ $poll_stations->poll_building_detail }}</option>
								@endforeach
							</select>
						...
						<table id="example0" class="table table-bordered tablefilter">
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ $slnoinpart }}</td>
									<td><a href="{{ url('/ro/voter-detail/'.$iCardNoEnc) }}">{{ $iCardNo }}</a></td>
									<td>{{ $name }}</td>
									<td>
									<?php
										if(($rlnType)=="F"){
