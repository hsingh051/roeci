				...
				<div class="panel panel-widget heightWidget">
					...
					<div class="panel-body">
						...
						<div class="noticeCandi">
							<form enctype="multipart/form-data" method="post" action="{{url('eci/updateObserver') }}" >
							...
							<input type="hidden" name="uidHide" value="<?php echo $obsuidEnc; ?>">
								...
								<div class="form-group{{ $errors->has('obDistEdit') ? ' has-error' : '' }}">
									<label class="form-label">Observer State</label>
									<select name="obDistEdit" type="file" class="form-control" required/>
										<option value="">Select</option>
										@foreach($distlist as $distlists)
										<?php $encDist=eci_encrypt($distlists->dist_code); ?>
										<option value="<?php echo $encDist; ?>" <?php if( ($distlists->dist_code)==($editObs->dist_code)){ echo "selected"; } ?>>{{ $distlists->dist_name }}</option>
										@endforeach
									</select>
									@if ($errors->has('obDistEdit'))
										<span class="help-block">
											<strong>{{ $errors->first('obDistEdit') }}</strong>
										</span>
									@endif
								</div>
