				...
				<div class="panel panel-widget heightWidget">
					...
					<div class="panel-body">
						<div class="noticeCandi">
							<form enctype="multipart/form-data" method="get" action="{{url('eci/p-1Sub') }}">
							<input type="hidden" name="_token" value="<?= csrf_token(); ?>">
								<div class="form-group{{ $errors->has('distCode') ? ' has-error' : '' }}">
									...
									<select id="district" name="distCode" class="form-control">
										...
										<?php
										$distCode=$distlists->dist_code;
										$distEnc=eci_encrypt($distCode);
										?>
										<option value="<?php echo $distEnc; ?>">{{ $distlists->dist_name }}</option>
										@endforeach
									</select>
